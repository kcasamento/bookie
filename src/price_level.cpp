...
bool PriceLevel::addOrder(Order o) {
  if (level_ != o.price) { return false; }
...
std::vector<Trade> PriceLevel::matchOrder(Order& o) {
  ...
  while (o.quantity > 0 && !orders_.empty()) {
    ...
    // capture the trade
    if (o.side == Side::Buy) {
      trades.push_back(Trade(o.id, in.id, level_, amt));
    } else {
      trades.push_back(Trade(in.id, o.id, level_, amt));
    }
...
void PriceLevel::cancelOrder(size_t id) {
  orders_.erase(std::remove_if(orders_.begin(), orders_.end(), 
      [id](const Order& o) { return o.id == id; }), orders_.end());
 return;
}
